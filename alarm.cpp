...
int Alarm::checkAppt(const QList<QString> mId)
{
    int a = apptModel->checkAppt(mId);
    ui->tableView->resizeRowsToContents();
...
int Alarm::checkAppt(const QList<inf> listAppt)
{
    int a = apptModel->checkAppt(listAppt);
    _cnt = 0;
    ui->tableView->resizeRowsToContents();
    return a;
}
