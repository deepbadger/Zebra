...
QString Messmodel::getMsg()
{
  QStringList lst;

  for (int i = 0; i < Infmail.size(); ++i) {
    inf_mail A = Infmail.at(i);
    lst.append(A.getname()+" <"+A.getfrom()+">\n"+tr("Subject: ")+ A.getsubject());
  }
...
mresult Messmodel::check(const QList<inf_mail> &list)
{
  //qint64 now = QDateTime::currentMSecsSinceEpoch();
  ...
  mresult r;
  r.setresult(Infmail.size(),toAppend.size(),toRemove.size());
  return r;
}
